...
class ITS_question {
    ...
    function __construct($student_id, $db_name, $table_name) {
        //=====================================================================//
        global $db_dsn, $db_name, $tb_name, $tb_tags, $db_table_user_state, $mimetex_path, $files_path;
...
---NEW---*/
    ...
    function render_data() {
        ...
        $db1 = '<tr><th colspan="2">TITLE</th><th>ANS</th><th>CATEGORY</th><th>QUESTION<br>config</b></th><th>ANSWERS<br>config</th></tr>' .
                '<tr>' .
                '<td class="' . $css . '" colspan="2">' . $this->createEditTable('title', $this->Q_title, $style) . '</td>' .
                ...
                '<td class="' . $css . '">' . $this->createEditTable('title', $this->Q_question_config, $style) . '</td>' .
                '<td class="' . $css . '">' . $this->createEditTable('answers', $this->Q_answers_config, $style) . '</td>' .               
                '</tr>';
        $db2 = '';
        ...
        switch ($this->Q_type) {
            case 'c':
             ...
             /*   $texts1  = $this->render_QUESTION_parts();
				...
				$weights = $this->Q_question_weights;
                $Nvals = $this->Q_vals;
                $db2 .= '<tr>';
                for($k=0,$l=0;$k<$this->Q_answers;$k++,$l++){
					$w=$k+1;
					$db2 .= '<th>Formula'.$w.'</th>'
						  . '<th>Text'.$w.'</th>'
						  . '<th>Weight'.$w.'</th>';
					$edit_tb[$k]  = $this->createEditTable('formula'.$w, $vals[$k], $style);
					$edit_tbl[$l] = $this->createEditTable('text'.$w, $texts['text'.($l+1)], $style);
					$edit_tbw[$l] = $this->createEditTable('weight'.$w, $weights['weight'.($l+1)], $style);
				}  
                $ChkVal = '<input type="button" name="ShowPreview" id="ShowPreview" value="Value(s)">';
                $db2 .= '<th>'.$ChkVal.'</th><th>Min value</th><th>Max value</th></tr>'
                      .'<tr>';
		  ...
		  for ($f = 0; $f < $Nvals; $f++) {
             	  $val_tb = $this->createEditTable('val' . ($f + 1), $vals['val' . ($f + 1)], $style);
                    $min_tb = $this->createEditTable('min_val' . ($f + 1), $vals['min_val' . ($f + 1)], $style);
                    $max_tb = $this->createEditTable('max_val' . ($f + 1), $vals['max_val' . ($f + 1)], $style);      //$answer_str .= '<font color="blue">'.$f.'</font> = '.$vals[$f].'<br>';
...
/*
                ...
                */
                ...
                $this->Q_answers_values = $answers;
                //$this->Q_weights_values = $weights;
                /** March 29 - edit table  - khyati  changes start**/
                $fields = 'text1';
				for($k=1;$k<$this->Q_answers;$k++){
				$fields .=', text'.($k+1);
				}
			...
			//	$this->Q_question_parts = mysql_fetch_assoc($res);
//die($this->Q_question_parts['text1']."sduhsa : ".$this->Q_answers."wsew :".$query);
/***************************************/
				$fields = 'weight1';
				for($k=1;$k<$this->Q_answers;$k++){
					$fields .=', weight'.($k+1);
				...
				$this->Q_question_weights = mysql_fetch_assoc($res);
					break;
					/** March 29 - edit table  - khyati  changes end**/
        }
    }
    //=====================================================================//
            ...
            //-------------------------------------------//
                ...
                switch ($mode) {
                    ...
                    //-------------------------------------------//
                        ...
                        // Config from DB
                        $query = 'SELECT comment FROM stats_' . $this->student_id . ' WHERE question_id=' . $this->Q_num . ' AND event<>"skip" ORDER BY id';
                ...
                for ($i = 1; $i <= $n; $i++) {
                    ...
                    $ans = $this->createEditTable('R' . (abs($R[$i - 1])), $R_answers["R" . (abs($R[$i - 1]))], $style);
                    $ans = $this->renderFieldCheck($ans);
                    /*
    ...
    //=====================================================================//
                ...
                //+++++++++++++++++++++++++++++++++++++++++++//
                    ...
                    switch ($this->Q_type) {
                      ...
                      //-------------------------------------------//
                          ...
                          // Khyatis changes
                            $ans = '<table id="ITS_Qans" class="ITS_Qans" n="' . $n . '" qtype="' . $this->Q_type . '">'
									. '<tr width=100%><td><LABEL >Number of Formulas</LABEL></td>'
        ...
        $buttons = '<div id="cancelDialog" class="ITS_button" style="float:right">Cancel</div>'.
				   '<div id="PreviewDialog" class="ITS_button" style="float:right">Show Preview</div>'.
    ...
    //=====================================================================//
        $nav = '<div id="toolsContainerToggle"><span>&raquo;&nbsp;Tools</span></div>'
				. '<div id="toolsContainer" style="display: none;">'
				. '<input type="button" class="' . $style . '" id="createQuestion" name="new"   value="New"   qid="' . $qid . '" qtype="' . $qtype . '">'
    ...
    function createEditTable($TargetName, $Target, $style) {
        ...
        $Table = '<table class="'.$style.'">'
                . '<tr>'
                . '<td class="' . $style . '">'
                . '<div id="ITS_' . $TargetName . '_TARGET" class="ITS_TARGET">'
                . $tb
                . '</div>'
                . '</td>'
                . '<td class="' . $style . '">'
