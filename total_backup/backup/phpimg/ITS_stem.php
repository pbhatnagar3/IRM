...
impulse: x[n] = 2*delta[n-1]
Callable: <img src="ITS_stem.php?t=i&d=1,2&x=n" class="ITS_pzplot">
...
// x/y-label
$font = round(2*log($Height/100)+3);
imagestring($img, $font, 0.95 * $Width, $yB + $unit / 2,$xlabel, $black);
imagestring($img, $font, $xB + $unit / 4, $unit / 4, $title, $black);
...
{
    for ($iw = 1; $iw < $w / $s; $iw++) {
        imageline($img, $iw * $s, 0, $iw * $s, $w, $color);
    }
    for ($ih = 1; $ih < $h / $s; $ih++) {
        imageline($img, 0, $ih * $s, $w, $ih * $s, $color);
    }
}
//----------------------------------------------------------//
