...
Last Revision: Aug-22-2012
//-----------------------------------------//
CALLS: 		 require_once("classes/ITS_user.php");
			 $x = new ITS_user($name);
			 $o = $x->add_user(...);
//=====================================================================*/
...
class ITS_user
{
    ...
    function add_user($first_name, $last_name, $username, $status){
        ...
        $query1 = 'INSERT INTO users (id,first_name,last_name,username,password,status) VALUES (' . $id[0] . ', ' . '"' . $first_name . '", ' . '"' . $last_name . '", ' . '"' . $username . '", ' . '"' . $username . '", ' . '"' . $status . '")';       
        $res =& $mdb2->exec($query1);
        
        $query2 = 'CREATE TABLE IF NOT EXISTS stats_' . $id[0] . ' (id int NOT NULL AUTO_INCREMENT, question_id int, concept_id int, current_chapter int, answered varchar(1024), score float, rating int, comment varchar(128),tags varchar(256), epochtime int(10) unsigned, duration int(11), event varchar(63), PRIMARY KEY (id), FOREIGN KEY (concept_id) REFERENCES concept (id) ON DELETE SET NULL )';
        $res =& $mdb2->exec($query2);
        $label = '<b>ADDED USER:  <a href="Profile.php?class=' . $status . '&sid=' . $id[0] . '&ch=1">' . $id[0] . '</a></b>';
        $str   = $label . '<p><div class="ITS_SQL">' . $query1 . '</div>' . '<div class="ITS_SQL">' . $query2 . '</div></p>';
        $mdb2->disconnect();
        return $str;
    }
    //=====================================================================//
    function render_courses()
    {
        ...
        $add = '<FORM id="add_class_Form" action="Course.php" method="POST">' . "<INPUT TYPE=file name=new_class>&nbsp;&nbsp;" . "<INPUT TYPE=BUTTON class=ITS_button onclick=\"ITS_form_submit(this.name,'" . $msg . "')\" VALUE='Add' NAME='add_class_Form'>" . "</form>";
        
        // Obtain top class name
        $course =& $mdb2->queryAll("SELECT * FROM course");
        
        $tb_course = '<table class="ITS_course">' . '<tr><th>NAME</th><th>TERM</th><th>YEAR</th><th></th></tr>';
        for ($i = 0; $i < count($course); $i++) {
            $name = preg_replace('/_/', ' ', $course[$i][1]);
            $del  = '<form id="delete_class_Form" action="Course.php" method="POST">' . '<INPUT TYPE=HIDDEN name="delete class" VALUE="' . $course[$i][1] . '">' . '<INPUT class="ITS_button" TYPE=SUBMIT ONCLICK=ITS_form_submit(this.name,"' . $msg . '") NAME=delete_class_Form VALUE="delete">' . '</form>';
            $tb_course .= '<tr><td>' . $name . '</td><td>' . $course[$i][2] . '</td><td>' . $course[$i][3] . '</td><td>' . $del . '</td></tr>';
        }
        $tb_course .= '<tr><td colspan="4">' . $add . '</td></tr>' . '</table>';
        $str = $tb_course;
