...
class ITS_configure
{
    ...
    function __construct($qid, $caption, $data, $weight, $image, $configuration, $mode)
    {
        ...
        // echo '<hr>' . time() . '<br>MODE: ' . $mode . ' - CONF: ' . $this->config;
                ...
                */
                ...
                switch ($mode) {
                    ...
                    case 0:
                        //-----------------------------//
                        for ($i = 1; $i <= $N; $i++) {
                            //echo htmlspecialchars($this->caption[$i]);
                    ...
                    case 2:
                        ...
                        for ($i = 1; $i <= $N; $i++) {
                            ...
                            $tb         = '<table class="ITS_ANSWER_BOXED"><tr>' . '<td class="ITS_ANSWER_BOXED_ALPH">' . $this->caption[$i] . '</td>' . '<td class="ITS_ANSWER_BOXED"><span class="' . $style . '">' . $answer[$i] . '</span></td>' . '<td><div class="CHOICE_WEIGHT">' . $weight[$i] . '</div></td>' . '<td><div class="CHOICE_ACTIVE">' . $image[$i] . '</div></td>' . '<td><form name="browser" action="server_browser.php"><input type="hidden" name="id" value="' . $qid . '"><input type="hidden" name="col_name" value="image' . $i . '"></form></td>' . '</tr></table>';
                            $answer_str .= $tb;
                        }
            ...
            case 4: //---- IMAGES ----//
                ...
                switch ($mode) {
                    ...
                    case 0:
                        //-----------------------------//
                        for ($i = 1; $i <= $N; $i++) {
                            $tb = '<table class="ITS_ANSWER_BOXED"><tr><td class="ITS_ANSWER_BOXED_ALPH">' . $this->caption[$i] . '</td><td class="ITS_ANSWER_BOXED"><span class="' . $style . '">' . $this->image[$i] . '</span></td></tr></table>';
                            $answer_str .= $tb;
                        }
                        break;
                    //-----------------------------//
                    case 1:
                        ...
                        //echo $style;
                        for ($i = 1; $i <= $N; $i++) {
                            $answer_str .= '<span name="ITS_MC" id="choice_' . $i . '_' . $N . '" class="' . $style . '" qid="' . $qid . '" n="' . $i . '">' . $this->image[$i] . '</span>';
                            //$answer_str .= $this->caption[$i].'<span class="' . $style . ' ANDTHIS">' . $this->image[$i] . '</span>';
                        }
                        break;
                    //-----------------------------//
                    case 1: // ACTIVE
                    ...
                    case 2:
                        ...
                        for ($i = 1; $i <= $N; $i++) {
                            $image[$i]  = $this->image[$i];
                            $weight[$i] = $this->weight[$i];
                            $answer_str .= '<span name="ITS" id="ITS" class="' . $style . '" qid="' . $qid . '" n="' . $i . '"><div class="ITS_ANSWER_IMG">' . $image[$i] . '</div></span><div class="CHOICE_WEIGHT" style="float:left">' . $weight[$i] . '</div>';
                        }
                        break;
                }
                //----------------//
