...
class ITS_survey
{
    ...
    function configure_lab()
    {
        ...
        if (func_num_args() == 0) {
            ...
            $this->_lab_sec_parts    = array(
                1,
                1,
                3
            );
            $this->_lab_sec_ques     = array(
                array(
                    1
                ),
                array(
        ...
        if ($this->_lab_num < 10) {
            $lab_num = '0' . $this->_lab_num;
        } else {
            $lab_num = $this->_lab_num;
        }
        switch ($this->_mode) {
            //-------------------------------
            case "lab":
    ...
    function lab_check()
    {
        ...
        if (!empty($completed[0])) {
            ...
            echo '<p><center><DIV class="ITS_MESSAGE">' . '<p>You have already completed <b>' . $this->_lab_title . '</b>.<p>' . '<a href="Home.php">back</a>' . '</DIV></center>';
        }
    }
    //=====================================================================//
    function record_lab()
    {
        ...
        echo '<p><center><DIV class="ITS_MESSAGE">' . '<p>Thanks, your <b>' . $this->_lab_title . '</b> answers have been recorded on ' . $time . '.<p>' . '<a href="Home.php">back</a><p>' . 'Please do not forget to logout.' . '</DIV></center>';
    }
    //=====================================================================//
    function record_lab_answers($ans_arr, $db_table_user_state)
    {
        ...
        if ($this->_lab_completed) {
            foreach ($ans_arr as $qNum => $qNum_array) {
                ...
                // CASE: MC: 
                if (is_array($qNum_array)) {
                    $ans = implode(",", $qNum_array);
                } else {
                    $ans = $qNum_array;
                }
    ...
    function render_lab()
    {
        ...
        switch ($this->_mode) {
            ...
            case "lab":
                //-------------------------------
                echo '<H3><div class="center">Lab #' . $this->_lab_num . ': ' . $this->_lab_title . '</div></H3>' . '<b>' . $sec_num . '  Warm-Up</b></p>' . '<form action="ITS_lab' . $lab_num . '.php" name="Warm-Up' . $this->_lab_num . '" method="post"><p>';
                break;
            //-------------------------------
            case "survey";
                //-------------------------------
                echo '<H3><div class="center">' . $this->_lab_title . '</div></H3>' . '<form action="ITS_pre_lab.php?activity=' . $this->_lab_num . '" name="survey" method="POST"><p>';
                //-------------------------------
        }
        ...
        */
        ...
        // SUBMIT BUTTON
        echo '<div class="center">' . '<input type="submit" name="submit" value="Submit ' . $this->_lab_title . '">' . '</div>' . '</form>';
    }
    //=====================================================================//
                        ...
                        case "survey";
                            ...
                            if (($this->_lab_ques_idx[$n] == 399) || ($this->_lab_ques_idx[$n] == 422)) {
                                ...
                                for ($i = 1; $i <= $Q->Q_answers; $i++) {
                                    ...
                                    $data       = array(
                                        "<span class=TextAlphabet>" . chr($i + 64) . ".</span>",
                                        $Q->Q_answers_data["answer" . $i],
                                        $hist[$this->_lab_ques_idx[$n]][$i - 1]
                                    );
        ...
        for ($k = 0; $k <= $this->_lab_sec - 1; $k++) {
            ...
            for ($n = 0; $n <= $this->_lab_sec_parts[$k] - 1; $n++) {
                ...
                for ($q = 0; $q <= $this->_lab_sec_ques[$k][$n] - 1; $q++) {
                    ...
                    $Q->load_DATA_from_DB($this->_lab_ques_idx[$n]);
                    //$Q->render_TITLE();
                    $pdf->Cell(40, 10, $Q->render_QUESTION());
                    $Q->get_ANSWERS_data_from_DB();
