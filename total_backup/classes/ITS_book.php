...
Author(s): Greg Krudysz |  Aug-19-2012
//=====================================================================*/
...
class ITS_book {
    ...
    //=====================================================================//
        global $db_dsn, $db_name, $tb_name, $db_table_user_state, $tex_path, $files_path;
        ...
        for ($i = 0; $i <= count($pars) - 1; $i++) {
            ...
            switch ($this->meta) {
                ...
                case 'image': // NO SCORE
                    ...
                    $fname = trim(str_replace('_5in','',$fname));
                    $img_source = 'ITS_FILES/BOOK_R/Chapter' . $chs . '/art/' . $fname . '.png';
                    
                    //echo $img_source.'<p>';
                    //die($img_source);
                    $caption = $pars[$i][4];
                    $caption = preg_replace('#(?<!\\\\)(\\$|\\\\)#','##', $caption); // \\\\$1
                    //echo $caption.'<br>';
                    $path = '/cgi-bin/mathtex.cgi?\large';
                    $rep  = '<img class="ITS_LaTeX" latex="${2}" src="' . $path . ' ${2}"/>';
                    $caption = preg_replace("/(##)(.*)?(##)/U",$rep,$caption);
                    //die($caption);
