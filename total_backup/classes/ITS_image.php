...
Last Revision: Jul-04-2012, Greg Krudysz
//====================================================================*/
class ITS_image{
    var $id;
    var $col_name;
    ...
    function __construct($id = 0, $col_name = 0){
        //======================================================//
        global $db_dsn, $db_name, $tb_name, $tb_images, $db_table_user_state, $tex_path, $files_path, $host;
        //var_dump($files_path);
    ...
    function get_related_images(){
        ...
        $query = 'SELECT qtype FROM ' . $this->tb_name . ' WHERE id=' . $this->id;
        $result = mysql_query($query, $this->con) or die(mysql_error());
        $row = mysql_fetch_assoc($result);
        
        if ($row) {
            $qtype = strtolower($row['qtype']);
        } else {
            die("No results for this question ID: " . $this->id);
        }
        
        $image_query = 'SELECT ' . $this->tb_images . '_id FROM ' . $this->tb_name . ' WHERE id=' . $this->id;
        //echo $image_query.'<br>';
        $result = mysql_query($image_query, $this->con) or die(mysql_error());
        $row = mysql_fetch_assoc($result);
        $x   = 0;
        if ($row && $row[$this->tb_images . '_id'] != '')
            $image_array[$x++] = $row[$this->tb_images . '_id'];
        ...
        if ($qtype == "mc") {
            ...
            $cols .= 'image' . $i;
            //echo $image_array[0];
            $image_query = 'SELECT ' . $cols . ' FROM ' . $this->tb_name . '_' . $qtype . ' WHERE '.$this->tb_name.'_id=' . $this->id;
            $result = mysql_query($image_query, $this->con) or die(mysql_error());
            $row = mysql_fetch_assoc($result);
    ...
    function get_all_images($page){
        ...
        //echo $page.' '.$min.' '.$max;
        $query = 'SELECT id,name,dir FROM ' . $this->tb_images . ' WHERE dir NOT RLIKE "images/question" AND name NOT LIKE "sm_%"';
        //die($query);
        ...
        for ($i = 0; $index < count($image_files) && $i < $this->rows; $i++) {
            ...
            for ($j = 0; $j < $this->columns && $index < count($image_files); $j++) { //
                ...
                */
                ...
                $table .= '<td id="' . $image_path . '">' . '<img class="img_sm" val="' . $j . '" src="' . $image_path . '" iid="' . $image_files[$index]['id'] . '"/></a></td>';
                $index++;
            }
            $table .= '</tr>';
        }
    ...
    function save_image($qid, $iid, $field_name){
        ...
        if ($row) {
            ...
            die("No results for this question ID: " . $qid);
        }
        
        switch ($field_name) {
            case $this->tb_images . '_id':
    ...
    //======================================================//
        ...
        if ($row) {
            ...
            die("No results for this question ID: " . $id);
        }
        ...
        $query = 'INSERT IGNORE INTO ' . $this->tb_images . ' (name,dir) VALUES("' . $name . '","' . $dir . '")';
        //die($query);
        $result = mysql_query($query, $con) or die(mysql_error());
        ...
        if ($iid == 0) {
            ...
            $query = 'SELECT id FROM ' . $this->tb_images . ' WHERE name="' . $name . '" AND dir="' . $dir . '"';
            $result = mysql_query($query, $con) or die(mysql_error());
            $row = mysql_fetch_assoc($result);
            if ($row) {
                $iid = $row['id'];
        ...
        //$query = 'UPDATE '.$this->tb_name.' SET image_id="'.$ques_dir.$filename.'" WHERE id='.$qid;  //die($query);
        
        //echo $_SERVER['HTTP_HOST'].' ---- '.rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
        //die();
        // -- redirect to Question page -- //	
        header("Location: http://" . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/../Question.php?qNum=" . $qid);
    }
    //======================================================//
}
?>
